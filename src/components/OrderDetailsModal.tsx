...
export default function OrderDetailsModal({ 
  ...
  const getStatusConfig = (status: string) => {
    const configs: Record<string, { label: string, color: string, ring: string, icon: any }> = {
      ...
      'confirmed': { label: 'Confirmed', color: 'bg-brand', ring: 'ring-cyan-100', icon: CheckCircle2 },
      'processing': { label: 'Processing', color: 'bg-brand', ring: 'ring-blue-100', icon: Zap },
      ...
      'returned': { label: 'Returned', color: 'bg-surface-hover0', ring: 'ring-slate-100', icon: History },
    };
        ...
        >
          ...
          <div className="h-16 sm:h-20 border-b border-border flex items-center justify-between px-4 sm:px-8 bg-surface/80 backdrop-blur-md sticky top-0 z-30">
            <div className="flex items-center gap-3 sm:gap-5">
              <div className={`w-10 h-10 rounded-xl ${statusConfig.color} flex items-center justify-center text-white shadow-subtle shadow-black/10`}>
                <StatusIcon size={18} strokeWidth={2.5} />
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm sm:text-lg font-bold text-primary truncate">#{order.orderNumber || order.id.slice(0, 8)}</h3>
                        ...
                        <React.Fragment key={s}>
                          <div className="flex flex-col items-center gap-2 relative">
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 ${
                              ...
                              isPastOrCurrent ? 'bg-slate-900 dark:bg-white text-white dark:text-black' : 'bg-surface text-muted border border-border'
                            }`}>
                            ...
                            <span className={`text-[8px] font-bold uppercase tracking-wider ${isPastOrCurrent ? 'text-primary' : 'text-muted'}`}>
                              {s}
                            </span>
                          </div>
                          {idx < arr.length - 1 && (
                            <div className={`flex-1 h-0.5 mx-1 rounded-full ${isPastOrCurrent && arr.indexOf(order.status.toLowerCase()) > idx ? 'bg-slate-900 dark:bg-white text-white dark:text-black' : 'bg-gray-200'}`} />
                          )}
                        </React.Fragment>
                      );
                    })}
                ...
                <div className="space-y-4">
                  ...
                  <div className="divide-y divide-border">
                    {order.items?.map((item: any, idx: number) => {
                      ...
                      return (
                        <div key={idx} className="flex items-center gap-4 py-4 hover:bg-surface-hover/50 px-2 rounded-xl transition-colors">
                          <div className="w-16 h-16 rounded-xl bg-surface-hover flex-shrink-0 overflow-hidden border border-border">
                            {productImage ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center text-muted"><Box size={24} /></div>
                            )}
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              {item.variant && <span className="text-[9px] font-bold bg-brand/10 text-brand px-1.5 py-0.5 rounded">{item.variant}</span>}
                              <span className="text-[10px] font-medium text-muted tracking-tight">Quantity: {item.quantity}</span>
                            </div>
                          </div>
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-[9px] font-medium text-muted">@{currencySymbol}{item.price}</p>
                          </div>
                        </div>
                      );
                    })}
              ...
              <div className="lg:col-span-4 bg-surface-hover/30 p-4 sm:p-8 space-y-10">
                ...
                <div className="space-y-6">
                   ...
                   <div className="space-y-4">
                      <div className="flex gap-4 items-start">
                        ...
                        <div>
                          <p className="text-[9px] font-bold text-muted uppercase tracking-wider mb-0.5">Full Name</p>
                      ...
                      <div className="flex gap-4 items-start">
                        ...
                        <div>
                          <p className="text-[9px] font-bold text-muted uppercase tracking-wider mb-0.5">Phone Number</p>
                          <p className="text-sm font-bold text-primary">{order.customerPhone}</p>
                        </div>
                      </div>
                      <div className="flex gap-4 items-start">
                        <div className="w-10 h-10 rounded-xl bg-surface border border-border flex items-center justify-center text-muted shadow-subtle">
                        ...
                        <div>
                          <p className="text-[9px] font-bold text-muted uppercase tracking-wider mb-0.5">Delivery Address</p>
                          <p className="text-[11px] font-medium text-secondary leading-normal">{order.customerAddress}</p>
                        </div>
                      </div>
                   </div>
                </div>
                ...
                <div className="bg-surface p-6 rounded-2xl border border-border shadow-subtle space-y-4">
                   <div className="flex justify-between items-center text-[11px] font-bold text-muted">
                     <span>Subtotal</span>
                     <span className="text-primary">{currencySymbol}{calculation.subtotal.toLocaleString()}</span>
                   </div>
                   <div className="flex justify-between items-center text-[11px] font-bold text-muted">
                     <span>Shipping</span>
                     <span className="text-primary">{currencySymbol}{calculation.delivery.toLocaleString()}</span>
                   </div>
                   ...
                   <div className="flex justify-between items-center pt-1">
                     <span className="text-xs font-black text-primary uppercase">Order Total</span>
                     <span className="text-lg font-black text-brand">{currencySymbol}{calculation.total.toLocaleString()}</span>
                   </div>
                   
                   <div className="pt-4 border-t border-border flex items-center justify-between">
                     <div>
                       <p className="text-[9px] font-bold text-muted uppercase tracking-wider mb-0.5">Due Amount</p>
                     ...
                     <div className="text-right">
                       <p className="text-[9px] font-bold text-muted uppercase tracking-wider mb-0.5">Paid</p>
                       <p className="text-xs font-bold text-secondary">{currencySymbol}{calculation.paid.toLocaleString()}</p>
                     </div>
                   </div>
                </div>
                ...
                <div className="space-y-4">
                   <div className="flex items-center justify-between px-1">
                      ...
                      <span className="text-[9px] font-bold text-muted">{customerHistory.length} Total</span>
                   </div>
                   <div className="space-y-3 max-h-[200px] overflow-y-auto no-scrollbar">
                      {loadingHistory ? (
                        ...
                        <div className="text-center py-6 bg-surface rounded-xl border border-dashed border-border">
                          <p className="text-[9px] font-bold text-muted uppercase">No other records</p>
                        </div>
                      ) : (
                        customerHistory.filter(h => h.id !== order.id).slice(0, 3).map((h, idx) => (
                           <div key={idx} className="bg-surface p-3 rounded-xl border border-border flex items-center justify-between">
                              <div>
                                 <p className="text-[10px] font-bold text-primary">#{h.orderNumber || h.id.slice(0, 8)}</p>
                              ...
                              <span className="text-xs font-bold text-primary">{currencySymbol}{h.totalAmount.toLocaleString()}</span>
                           </div>
                        ))
                      )}
                   </div>
